...
import { Search, Loader2, ShieldCheck, Database, RefreshCw, AlertTriangle, History, X, ChevronRight } from "lucide-react"
...
export default function IntuneBackupDashboard() {
  ...
  const fetchBackups = async (category?: string) => {
    setBackupsLoading(true);
    try {
      const url = category
        ? `${BASE_URL}/backups?${AUTH_QUERY}&category=${category}`
        : `${BASE_URL}/backups?${AUTH_QUERY}`;
  ...
  const filteredBackups = backups.filter(b => {
    const name = b.policyName || "";
    return name.toLowerCase().includes(backupSearch.toLowerCase());
  });
  ...
  return (
    <div className="p-8 space-y-8 bg-slate-50 min-h-screen font-sans text-slate-900">
          ...
          {filteredPolicies.length > 0 ? (
            filteredPolicies.map((policy) => (
              ...
              >
                ...
                <p className="text-xs text-slate-400 font-mono mb-6 break-all opacity-60">
                  {policy.id}
                </p>
      ...
      {/* Backup History Slide-over Panel */}
      <AnimatePresence>
        {showHistory && (
          <>
            {/* Backdrop */}
            ...
            >
              ...
              <div className="flex justify-between items-start p-6 border-b border-slate-200">
                <div>
                  <h2 className="text-xl font-extrabold text-slate-900">Backup History</h2>
                  <p className="text-sm text-slate-500 mt-1">
                    {selectedPolicy
                      ? `Showing backups for: ${selectedPolicy.displayName || selectedPolicy.name}`
                      : "All backed up policies"}
                  </p>
                </div>
                <button
                  onClick={() => setShowHistory(false)}
              ...
              <div className="px-6 py-4 border-b border-slate-100">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="pl-10 w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
                    value={backupSearch}
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-3">
                {backupsLoading ? (
                  <div className="flex flex-col items-center justify-center py-20 gap-4">
                    ...
                    <p className="text-slate-400 text-sm animate-pulse">Loading backup history...</p>
                  </div>
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div className="flex-1 min-w-0">
                            ...
                            <h4 className="font-bold text-slate-800 text-sm truncate">{backup.policyName}</h4>
                            <p className="text-xs text-slate-400 font-mono mt-1 truncate opacity-60">{backup.blobPath}</p>
                          </div>
                          <ChevronRight className="h-4 w-4 text-slate-300 ml-3 mt-1 flex-shrink-0" />
                        </div>
                        <div className="mt-3 pt-3 border-t border-slate-200 flex justify-between items-center">
                          ...
                          <span className="text-[10px] font-mono text-slate-300">{backup.backupId.substring(0, 8)}...</span>
                        </div>
                      </motion.div>
                    ))
                ) : (
      ...
      <footer className="pt-8 border-t border-slate-200 flex justify-between text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">
        ...
        <span>Version 1.0.7 - 2026 Build</span>
      </footer>
    </div>
  );
}
